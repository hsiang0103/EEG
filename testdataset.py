...
class TestDataset(Dataset):
    ...
    def split_windows(self)->None:
        ...
        for which_file, (file, summary) in enumerate(zip(edf_files, self.summary_dictory)):
            ...
            signals_tensor = signals_tensor[:24] if signals_tensor.shape[0] > 23 else signals_tensor
            ...
            while end_point <= signals_tensor.shape[2]:
                ...
                for idx in range(len(seizures_intervals)//2):
                    # print(seizures_intervals[idx * 2])
                    # print(seizures_intervals[idx * 2 + 1])
                    if (seizures_intervals[idx * 2] * self.freq <= start_point and start_point <= seizures_intervals[idx * 2 + 1] * self.freq) or \
                        (seizures_intervals[idx * 2] * self.freq <= end_point and end_point <= seizures_intervals[idx * 2 + 1] * self.freq):
                        label = 1
                    else:
                        label = 0
                # append data
                self.datas[data_idx] = (signals_tensor[:, :, start_point : end_point], label)
                data_idx += 1
    ...
    def combine_dataset(self, datasets_list):
        """
        self dataset = self dataset + a list of other datasets.
        """
        #total_num_needed_conbine = 0
        #for single_dataset in datasets_list:
        #    total_num_needed_conbine += len(single_dataset)
        
        for idx, single_dataset in enumerate(datasets_list):
            for data in single_dataset:
                self.datas.append(data)
            
            processbar(
                now_process=idx + 1, 
                all=len(datasets_list), 
                total_len=30, 
                info=f"[{idx + 1}/{len(datasets_list)}] datas combined."
            )
